...
const RUNTIME_LOG_FILE = path.join(RUNTIME_LOG_DIR, 'server-runtime.log');
const MOBILE_CONNECT_REQUESTS_FILE = path.join(DB_DIR, 'mobile-connect-requests.json');
const API_KEYS_FILE = path.join(DB_DIR, 'api-keys.json');
const MAINTENANCE_STATE_FILE = path.join(DB_DIR, 'maintenance-state.json');
...
const ADMIN_GROUPS_KV_KEY = 'ds.security.admin-groups';
...
const CONNECTED_CLIENT_TTL_MS = 2 * 60 * 1000;
const SYSTEM_USAGE_SAMPLE_WINDOW_MS = 2500;
const DOWNLOAD_PLATFORMS = ['windows', 'linux', 'android'];
const PERMISSION_CATALOG = [
  { key: 'dashboard', label: 'Dashboard' },
  { key: 'screens', label: 'Écrans' },
  { key: 'assets', label: 'Médias' },
  ...
  { key: 'updates', label: 'Mise à jour' },
  { key: 'downloads', label: 'Téléchargements' }
];
const PERMISSION_KEY_SET = new Set(PERMISSION_CATALOG.map((item) => item.key));
const adminSessions = new Map();
const panelConnectionLastSeenAt = new Map();
const connectedPanelClients = new Map();
const PANEL_CONNECTION_COOLDOWN_MS = 5 * 60 * 1000;
let systemUsageLastSample = null;
let systemUsageBroadcastTimer = null;
let systemUsageBroadcastRunning = false;

const NETWORK_VENDOR_PRIORITY_KEYWORDS = ['realtek', 'broadcom', 'mediatek'];
const NETWORK_WIFI_KEYWORDS = ['wifi', 'wi-fi', 'wlan', 'wireless'];
const NETWORK_VM_KEYWORDS = ['hyper-v', 'hyperv', 'vmware', 'virtualbox', 'qemu'];
const updateState = {
...
let fatalHandlerAttached = false;
...
function ensureJsonFile(filePath, defaultValue) {
  try {
    ...
    // ignore bootstrap failures for optional JSON files
  }
}
...
function writeJsonFile(filePath, value) {
  try {
    ...
    // ignore write errors for non-critical JSON files
  }
}
...
function loadApiKeys() {
  const rows = readJsonFile(API_KEYS_FILE, []);
  if (!Array.isArray(rows)) return [];
  return rows
    .filter((row) => row && typeof row === 'object')
    .map((row) => ({
      id: String(row.id || '').trim() || crypto.randomUUID(),
      name: String(row.name || 'mobile-client').trim(),
      ...
      scopes: Array.isArray(row.scopes) ? row.scopes.map((scope) => String(scope || '').trim()).filter(Boolean) : ['admin-api'],
    ...
    .filter((row) => row.keyHash.length > 0);
}

function saveApiKeys(rows) {
  writeJsonFile(API_KEYS_FILE, Array.isArray(rows) ? rows : []);
}

function createApiKeyRecord({ name, createdBy = 'system', scopes = ['admin-api'], expiresAt = null } = {}) {
  const id = crypto.randomUUID();
  const rawSecret = crypto.randomBytes(32).toString('base64url');
  const plainKey = `dk_live_${rawSecret}`;
  const prefix = plainKey.slice(0, 14);
  const record = {
    id,
    name: String(name || 'mobile-client').trim() || 'mobile-client',
    prefix,
    keyHash: hashApiKey(plainKey),
    createdAt: nowIso(),
    createdBy,
    scopes: Array.isArray(scopes) && scopes.length > 0 ? scopes : ['admin-api'],
    lastUsedAt: null,
    revokedAt: null,
...
function findApiKeyRecordByToken(token) {
  const secret = String(token || '').trim();
  if (!secret || !secret.startsWith('dk_live_')) {
    return null;
  }
  const now = Date.now();
  const rows = loadApiKeys();
  const hashed = hashApiKey(secret);
  const match = rows.find((row) => row.keyHash === hashed && !row.revokedAt);
  if (!match) return null;
  if (match.expiresAt) {
    const expiration = new Date(match.expiresAt).getTime();
...
function getApiKeyFromRequest(req) {
  ...
  if (bearer && bearer.startsWith('dk_live_')) return bearer;
  return '';
}

function loadMobileConnectRequests() {
  const rows = readJsonFile(MOBILE_CONNECT_REQUESTS_FILE, []);
  if (!Array.isArray(rows)) return [];
  return rows
    .filter((row) => row && typeof row === 'object')
    .map((row) => ({
      id: String(row.id || '').trim() || crypto.randomUUID(),
      appName: String(row.appName || 'Digital Kiosk App').trim(),
      platform: String(row.platform || 'unknown').trim(),
      callbackUrl: typeof row.callbackUrl === 'string' ? row.callbackUrl : '',
      state: String(row.state || '').trim(),
      pollToken: String(row.pollToken || '').trim(),
      approvalCode: String(row.approvalCode || '').trim(),
      status: ['pending', 'approved', 'rejected', 'exchanged'].includes(String(row.status || 'pending'))
        ? String(row.status)
        : 'pending',
      createdAt: typeof row.createdAt === 'string' ? row.createdAt : nowIso(),
      updatedAt: typeof row.updatedAt === 'string' ? row.updatedAt : nowIso(),
      approvedBy: typeof row.approvedBy === 'string' ? row.approvedBy : null,
      apiKeyId: typeof row.apiKeyId === 'string' ? row.apiKeyId : null,
      apiKeyPreview: typeof row.apiKeyPreview === 'string' ? row.apiKeyPreview : null,
      rejectReason: typeof row.rejectReason === 'string' ? row.rejectReason : null,
      exchangedAt: typeof row.exchangedAt === 'string' ? row.exchangedAt : null
    }))
    .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
}

function saveMobileConnectRequests(rows) {
  writeJsonFile(MOBILE_CONNECT_REQUESTS_FILE, Array.isArray(rows) ? rows : []);
}

function loadMaintenanceState() {
  const value = readJsonFile(MAINTENANCE_STATE_FILE, { enabled: false, reason: null, updatedAt: null });
  return {
...
function setMaintenanceMode(enabled, reason = null) {
  ...
  writeJsonFile(MAINTENANCE_STATE_FILE, {
    enabled: maintenanceMode,
    reason: typeof reason === 'string' && reason.trim() ? reason.trim() : null,
    updatedAt: nowIso()
  });
  appendRuntimeOnlyLog(
    maintenanceMode ? 'warning' : 'info',
    'server.maintenance',
    maintenanceMode ? 'Mode maintenance activé.' : 'Mode maintenance désactivé.',
    { reason: reason || null }
  );
}

function isPathAllowedDuringMaintenance(pathname) {
  if (pathname === '/updater') return true;
  if (pathname.startsWith('/api/system/update/')) return true;
  if (pathname === '/api/system/maintenance/status') return true;
  if (pathname === '/api/system/maintenance/toggle') return true;
  if (pathname === '/api/health') return true;
  if (pathname.startsWith('/api/auth/')) return true;
  if (pathname.startsWith('/api/mobile/connect/')) return true;
  return false;
}
    ...
    return parsed
      .map((entry) => {
        ...
        return {
          ...
          groups: sanitizeGroupIds(entry?.groups),
          permissions: sanitizePermissionMatrix(entry?.permissions, role)
        };
      })
      .filter(Boolean);
  } catch {
    ...
    return parsed
      .map((entry) => {
        ...
        return {
          ...
          permissions: sanitizePermissionMatrix(entry?.permissions, 'viewer'),
          createdAt: typeof entry?.createdAt === 'string' ? entry.createdAt : nowIso(),
          updatedAt: typeof entry?.updatedAt === 'string' ? entry.updatedAt : nowIso()
        };
      })
...
function buildDefaultPermissionMatrix(role = 'viewer') {
  const resolvedRole = sanitizeAdminRole(role);
  if (resolvedRole === 'admin') {
    const full = {};
    for (const key of PERMISSION_KEY_SET) {
      full[key] = { read: true, write: true };
    }
    return full;
  }
  ...
  const viewerDefaults = {
    ...
    screens: { read: true, write: false },
    assets: { read: true, write: false },
    playlists: { read: true, write: false },
    layouts: { read: true, write: false },
    ...
    updates: { read: false, write: false },
...
function sanitizePermissionMatrix(raw, role = 'viewer') {
  const defaults = buildDefaultPermissionMatrix(role);
  ...
  for (const key of PERMISSION_KEY_SET) {
    ...
    next[key] = {
      read: Boolean(node.read ?? defaults[key].read),
      write: Boolean(node.write ?? defaults[key].write)
    };
  }
...
function upsertAdminUserRecord(users, account, lastLoginAt) {
  ...
  const role = ADMIN_ROLES.has(String(account.role || '').trim().toLowerCase())
  ...
  const merged = {
    ...
    groups: sanitizeGroupIds(account.groups),
    permissions: sanitizePermissionMatrix(account.permissions, role)
  };
...
function isPublicApiPath(pathname, method = 'GET') {
  ...
  if (pathname === '/api/system/maintenance/status' && method === 'GET') return true;
  if (pathname === '/api/storage/policy' && method === 'GET') return true;
  if (pathname.startsWith('/api/auth/')) return true;
  if (pathname === '/api/mobile/connect/init' && method === 'POST') return true;
  if (pathname === '/api/mobile/connect/status' && method === 'GET') return true;
  if (pathname === '/api/mobile/connect/exchange' && method === 'POST') return true;
  ...
  if (pathname === '/api/monitoring/fleet' && method === 'GET') return true;
  if (pathname === '/api/monitoring/connected-clients' && method === 'GET') return true;
...
function setupSystemSyncWebSocketServer(server) {
  ...
  wss.on('connection', (ws) => {
    ...
    try {
      ...
      void sampleSystemUsage().then((usage) => {
...
function ensureRuntimeLogFile() {
  try {
    if (!fs.existsSync(RUNTIME_LOG_DIR)) {
      fs.mkdirSync(RUNTIME_LOG_DIR, { recursive: true });
    }
    if (!fs.existsSync(RUNTIME_LOG_FILE)) {
      fs.writeFileSync(RUNTIME_LOG_FILE, '', 'utf-8');
    }
  } catch {
    // ignore runtime log bootstrap failures
  }
}

function appendRuntimeLogLine(level, source, message, details) {
...
function appendLogEntry(db, partial) {
  ...
  appendRuntimeLogLine(entry.level, entry.source, entry.message, entry.details);
  return entry;
}

function appendRuntimeOnlyLog(level, source, message, details) {
  appendRuntimeLogLine(level, source, message, details);
  readDb()
    .then(async (db) => {
      appendLogEntry(db, {
        type: 'system',
    ...
    .catch(() => {
      appendRuntimeLogLine('error', 'server.runtime-log', 'Impossible de persister le log runtime en base.', {
        source,
        message
      });
    });
...
function attachProcessSafetyHandlers() {
  if (fatalHandlerAttached) {
    return;
  }

  fatalHandlerAttached = true;

  process.on('uncaughtException', (error) => {
    const details = serializeError(error);
    appendRuntimeOnlyLog('error', 'process.uncaughtException', details.message || 'Uncaught exception', details);
    console.error('[fatal] uncaughtException', error);
  });

  process.on('unhandledRejection', (reason) => {
    const details = serializeError(reason);
    appendRuntimeOnlyLog('error', 'process.unhandledRejection', details.message || 'Unhandled rejection', details);
    console.error('[fatal] unhandledRejection', reason);
  });

  process.on('SIGINT', () => {
    appendRuntimeOnlyLog('warning', 'process.signal', 'Signal SIGINT reçu, arrêt du serveur demandé.', {});
    if (serverInstance) {
  ...
  process.on('SIGTERM', () => {
    appendRuntimeOnlyLog('warning', 'process.signal', 'Signal SIGTERM reçu, arrêt du serveur demandé.', {});
    if (serverInstance) {
      serverInstance.close(() => process.exit(0));
      return;
    }
...
function buildLogMeta(req, extra = {}) {
  ...
  return {
    ...
    browser: detectBrowserFromUserAgent(ua),
...
function detectBrowserFromUserAgent(uaRaw) {
  const ua = String(uaRaw || '').toLowerCase();
  if (!ua) return 'unknown';
...
function summarizePayloadKeys(payload) {
  ...
  const SENSITIVE_KEYS = new Set(['password', 'token', 'apiKey', 'authorization', 'secret']);
  return Object.keys(payload)
    .filter((key) => !SENSITIVE_KEYS.has(String(key).toLowerCase()))
    .slice(0, 40);
}

function shouldLogPanelConnection(req, session) {
  if (!session) return false;
  const ip = getAdvertisedClientIp(req);
  const ua = String(req.headers['user-agent'] || '');
  const fingerprint = `${session.username || 'unknown'}|${ip}|${detectOsFromUserAgent(ua)}|${detectBrowserFromUserAgent(ua)}`;
  const now = Date.now();
  const previous = Number(panelConnectionLastSeenAt.get(fingerprint) || 0);
  if (now - previous < PANEL_CONNECTION_COOLDOWN_MS) {
    return false;
  }
  panelConnectionLastSeenAt.set(fingerprint, now);
  return true;
}

async function logPanelConnection(req, session, via = 'session') {
  if (!shouldLogPanelConnection(req, session)) return;
  try {
    const db = await readDb();
    appendLogEntry(db, {
      type: 'auth',
      level: 'info',
      source: 'api.panel.connection',
      message: 'Connexion au panel détectée',
      details: buildLogMeta(req, {
    ...
    appendRuntimeOnlyLog('error', 'api.panel.connection', 'Impossible de journaliser la connexion panel.', {
...
function sanitizeAdminUserForResponse(user) {
  return {
    ...
    groups: sanitizeGroupIds(user.groups),
    permissions: sanitizePermissionMatrix(user.permissions, user.role)
  };
}

function sanitizeAdminGroupForResponse(group) {
  return {
    id: String(group.id || '').trim() || crypto.randomUUID(),
    name: String(group.name || '').trim(),
    description: String(group.description || '').trim(),
    createdAt: group.createdAt,
    updatedAt: group.updatedAt,
    permissions: sanitizePermissionMatrix(group.permissions, 'viewer')
  };
}

function mergePermissionMatrices(base, overlay) {
  const merged = sanitizePermissionMatrix(base, 'viewer');
  const extra = sanitizePermissionMatrix(overlay, 'viewer');
  for (const key of PERMISSION_KEY_SET) {
    merged[key] = {
      read: Boolean(merged[key]?.read || extra[key]?.read),
      write: Boolean(merged[key]?.write || extra[key]?.write)
    };
  }
  return merged;
}

function resolveUserGroupsAndPermissions(db, username) {
  const normalizedUsername = normalizeUsername(username);
  const users = parseAdminUsers(db.kv[ADMIN_USERS_KV_KEY]);
  const groups = parseAdminGroups(db.kv[ADMIN_GROUPS_KV_KEY]);
  const user = users.find((item) => item.username === normalizedUsername) || null;
  if (!user) {
    return null;
  }

  const role = sanitizeAdminRole(user.role);
  if (role === 'admin') {
    return {
      user,
      groups: groups.filter((group) => sanitizeGroupIds(user.groups).includes(group.id)),
      permissions: buildDefaultPermissionMatrix('admin')
    };
  }

  let permissions = buildDefaultPermissionMatrix(role);
  const userGroupIds = sanitizeGroupIds(user.groups);
  const userGroups = groups.filter((group) => userGroupIds.includes(group.id));

  for (const group of userGroups) {
    permissions = mergePermissionMatrices(permissions, group.permissions);
  }

  permissions = mergePermissionMatrices(permissions, user.permissions);
...
function inferPermissionKeyFromPath(pathname) {
  const path = String(pathname || '').trim();
  if (!path.startsWith('/api/')) return null;
  if (path.startsWith('/api/screens')) return 'screens';
  if (path.startsWith('/api/assets')) return 'assets';
  if (path.startsWith('/api/playlists')) return 'playlists';
  if (path.startsWith('/api/layouts')) return 'layouts';
  if (path.startsWith('/api/events') || path.startsWith('/api/rooms') || path.startsWith('/api/meetings') || path.startsWith('/api/activity')) return 'calendar';
  if (path.startsWith('/api/logs') || path.startsWith('/api/audit')) return 'logs';
  if (path.startsWith('/api/monitoring') || path.startsWith('/api/ops')) return 'monitoring';
  if (path.startsWith('/api/alerts')) return 'alerts';
  if (path.startsWith('/api/system/update')) return 'updates';
  if (path.startsWith('/api/system/maintenance') || path.startsWith('/api/system/kv') || path.startsWith('/api/storage')) return 'settings';
  if (path.startsWith('/api/auth/groups')) return 'groups';
  if (path.startsWith('/api/auth/users')) return 'users';
  if (path.startsWith('/api/system/download') || path.startsWith('/api/downloads')) return 'downloads';
  return null;
}

function inferPermissionMode(method) {
  const upper = String(method || 'GET').toUpperCase();
  if (upper === 'GET' || upper === 'HEAD' || upper === 'OPTIONS') return 'read';
  return 'write';
}

function hasAdminPermission(req, permissionKey, mode = 'read') {
  const role = sanitizeAdminRole(req?.adminSession?.role);
  if (role === 'admin') return true;
  const matrix = req?.adminPermissions;
  if (!matrix || typeof matrix !== 'object') return false;
  const node = matrix[permissionKey];
  if (!node || typeof node !== 'object') return false;
  if (mode === 'write') return Boolean(node.write);
  return Boolean(node.read);
}

function normalizeConnectedClientFingerprint(ip, ua, username) {
  return `${String(ip || '').trim()}|${String(ua || '').trim()}|${String(username || '').trim()}`;
}

function upsertConnectedPanelClient(req, session, via = 'session', channel = 'http') {
  if (!session) return;
  const ip = getAdvertisedClientIp(req);
  const ua = String(req.headers['user-agent'] || '').trim();
  const now = nowIso();
  const username = session.username || 'unknown';
  const fingerprint = normalizeConnectedClientFingerprint(ip, ua, username);
  const existing = connectedPanelClients.get(fingerprint);
  connectedPanelClients.set(fingerprint, {
    ...
    lastSeenAt: now,
    lastPath: String(req.path || req.originalUrl || '').trim() || '/'
  });
}

function pruneConnectedPanelClients() {
  const now = Date.now();
  for (const [key, value] of connectedPanelClients.entries()) {
    const ts = new Date(value.lastSeenAt || 0).getTime();
    if (!Number.isFinite(ts) || (now - ts) > CONNECTED_CLIENT_TTL_MS) {
      connectedPanelClients.delete(key);
    }
  }
}

function listConnectedPanelClients() {
  pruneConnectedPanelClients();
  return Array.from(connectedPanelClients.values())
...
function classifyNetworkDescriptor(descriptor) {
  const lower = String(descriptor || '').toLowerCase();
  if (containsKeyword(lower, NETWORK_WIFI_KEYWORDS)) return 'wifi';
  if (containsKeyword(lower, NETWORK_VENDOR_PRIORITY_KEYWORDS)) return 'vendor';
  if (containsKeyword(lower, NETWORK_VM_KEYWORDS)) return 'vm';
  return 'other';
}

function normalizeNetworkPreference(value) {
  const raw = String(value || '').trim();
  if (!raw) return 'auto';
  return raw;
}

function scoreNetworkInterfaceCandidate(candidate, context) {
  const usageMbps = Number(candidate.totalMbps || 0);
  const descriptor = String(candidate.descriptor || '').toLowerCase();
  const category = classifyNetworkDescriptor(descriptor);

  let score = usageMbps * 100;
  if (category === 'wifi') score += 5000;
  if (category === 'vendor') score += 4200;
  if (category === 'vm') score += context.preferVirtual ? 7000 : 800;
  if (String(candidate.operstate || '').toLowerCase() === 'up') score += 500;
  if (candidate.defaultRoute) score += 300;

  return score;
}

function selectPreferredNetworkInterface(candidates, preferenceRaw, context) {
  if (!Array.isArray(candidates) || candidates.length === 0) return null;

  const preference = normalizeNetworkPreference(preferenceRaw);
  const lowerPreference = preference.toLowerCase();

  if (lowerPreference !== 'auto') {
    const manual = candidates
      .filter((candidate) => {
        const iface = String(candidate.iface || '').toLowerCase();
        const descriptor = String(candidate.descriptor || '').toLowerCase();
        return iface === lowerPreference || descriptor.includes(lowerPreference);
      })
      .sort((a, b) => Number(b.totalMbps || 0) - Number(a.totalMbps || 0))[0];

    if (manual) {
      return { selected: manual, mode: 'manual' };
    }
  }

  const ranked = [...candidates]
    .sort((a, b) => scoreNetworkInterfaceCandidate(b, context) - scoreNetworkInterfaceCandidate(a, context));
...
async function sampleSystemUsage() {
  const now = Date.now();
  const [loadInfo, memoryInfo, networkStatsRaw, networkInterfacesRaw, systemInfo, db] = await Promise.all([
  ...
  const systemConfig = db ? getSystemConfig(db) : {};
  const interfacePreference = normalizeNetworkPreference(systemConfig?.networkInterfacePreference || 'auto');
  const preferVirtual = Boolean(systemInfo?.virtual);

  const networkStats = Array.isArray(networkStatsRaw) ? networkStatsRaw : [];
  const networkInterfaces = Array.isArray(networkInterfacesRaw) ? networkInterfacesRaw : [];
  const interfaceMetaMap = new Map(
    networkInterfaces.map((iface) => [
      String(iface.iface || '').toLowerCase(),
      iface
    ])
  );

  const candidates = networkStats
    .map((row) => {
      const iface = String(row?.iface || '').trim();
      const key = iface.toLowerCase();
      const meta = interfaceMetaMap.get(key) || {};
      const rxMbps = Math.max(0, Number(row?.rx_sec || 0) * 8 / 1000000);
      const txMbps = Math.max(0, Number(row?.tx_sec || 0) * 8 / 1000000);
      ...
      return {
        ...
        operstate: String(meta?.operstate || row?.operstate || ''),
        defaultRoute: Boolean(meta?.default),
    ...
    .filter((candidate) => candidate.iface && !candidate.internal);

  const selection = selectPreferredNetworkInterface(candidates, interfacePreference, { preferVirtual });
  const selectedNetwork = selection?.selected || null;

  const memoryTotal = Number(memoryInfo?.total || os.totalmem() || 0);
  const memoryFree = Number(memoryInfo?.available || os.freemem() || 0);
  const memoryUsed = Math.max(0, memoryTotal - memoryFree);
  const memoryPercent = memoryTotal > 0 ? (memoryUsed / memoryTotal) * 100 : 0;
  const cpuPercent = Math.max(0, Math.min(100, Number(loadInfo?.currentLoad || 0)));
  const networkMbps = Number(selectedNetwork?.totalMbps || 0);
  const availableInterfaces = candidates
    ...
    .map((candidate) => ({
      ...
      operstate: candidate.operstate,
      defaultRoute: candidate.defaultRoute,
      virtual: candidate.virtual,
      rxMbps: Number(candidate.rxMbps.toFixed(3)),
      txMbps: Number(candidate.txMbps.toFixed(3)),
      totalMbps: Number(candidate.totalMbps.toFixed(3))
    }));

  systemUsageLastSample = { at: now };

  return {
    at: nowIso(),
    cpuPercent: Number(cpuPercent.toFixed(2)),
    memoryPercent: Number(memoryPercent.toFixed(2)),
    networkMbps: Number(networkMbps.toFixed(3)),
    memoryTotalBytes: memoryTotal,
    memoryUsedBytes: memoryUsed,
    network: {
      selectionMode: selection?.mode || 'auto',
      preference: interfacePreference,
      selectedInterface: selectedNetwork
        ? {
            iface: selectedNetwork.iface,
            descriptor: selectedNetwork.descriptor,
            type: selectedNetwork.type,
            vendor: selectedNetwork.vendor,
            operstate: selectedNetwork.operstate,
            rxMbps: Number(selectedNetwork.rxMbps.toFixed(3)),
            txMbps: Number(selectedNetwork.txMbps.toFixed(3)),
            totalMbps: Number(selectedNetwork.totalMbps.toFixed(3))
          }
        : null,
      availableInterfaces
    }
...
function startSystemUsageBroadcastLoop() {
  if (systemUsageBroadcastTimer) return;

  systemUsageBroadcastTimer = setInterval(async () => {
    if (systemUsageBroadcastRunning) return;
    if (systemSyncClients.size === 0) return;

    systemUsageBroadcastRunning = true;
    try {
      const usage = await sampleSystemUsage();
      broadcastSystemSync({
        type: 'system-usage',
        revision: systemSyncRevision,
        at: usage.at,
      ...
      systemUsageBroadcastRunning = false;
    }
  }, 1000);
}

function stopSystemUsageBroadcastLoop() {
  if (!systemUsageBroadcastTimer) return;
  clearInterval(systemUsageBroadcastTimer);
  systemUsageBroadcastTimer = null;
}

function buildPublicBaseUrl(req) {
  const host = String(req.headers['x-forwarded-host'] || req.headers.host || `127.0.0.1:${PORT}`).trim();
  const proto = String(req.headers['x-forwarded-proto'] || (req.secure ? 'https' : 'http')).split(',')[0].trim() || 'http';
  return `${proto}://${host}`;
}

function resolveDownloadTarget(platform) {
  const key = String(platform || '').trim().toLowerCase();
  if (!DOWNLOAD_PLATFORMS.includes(key)) return '';
  const envKey = `DOWNLOAD_${key.toUpperCase()}_URL`;
  ...
  const items = screens.map((screen) => {
    ...
    return {
      ...
      telemetry: {
        ...
        networkMbps: Number(screen.networkMbps || 0) || 0,
        networkInterface: String(screen.networkInterface || '').trim(),
...
function buildUpcomingMeetings(events, rooms) {
  ...
  return events
    ...
    .map((event, index) => {
      ...
      const eventId = event.id || `meeting-${index}`;
      return {
        id: eventId,
        title: event.title || 'Réunion',
        startAt: event.startAt || event.start || null,
        endAt: event.endAt || event.end || null,
        roomId: event.roomId || null,
        roomNumber: event.roomNumber || room?.number || '',
...
app.use((req, res, next) => {
  if (maintenanceMode && !isPathAllowedDuringMaintenance(req.path)) {
    return res.status(503).json({
      ok: false,
      error: 'maintenance-mode',
      message: 'Le système est temporairement en maintenance.'
    });
  }
...
app.use(async (req, res, next) => {
  ...
  if (apiKey) {
    const keyRecord = findApiKeyRecordByToken(apiKey);
    if (keyRecord && keyRecord.scopes.includes('admin-api')) {
      const dbSnapshot = await readDb();
      const resolved = resolveUserGroupsAndPermissions(dbSnapshot, keyRecord.createdBy || 'api-key');
      req.adminSession = {
        token: keyRecord.id,
        username: keyRecord.createdBy || 'api-key',
        role: sanitizeAdminRole(resolved?.user?.role || 'admin'),
        lastActivityAt: Date.now()
      };
      req.adminPermissions = resolved?.permissions || buildDefaultPermissionMatrix(req.adminSession.role);
      req.adminGroups = (resolved?.groups || []).map(sanitizeAdminGroupForResponse);
      req.apiKeySession = {
        keyId: keyRecord.id,
        keyName: keyRecord.name,
        scopes: keyRecord.scopes
      };
      void logPanelConnection(req, req.adminSession, 'api-key');
      upsertConnectedPanelClient(req, req.adminSession, 'api-key', 'http');
      return next();
    }
  }
  ...
  {
    const dbSnapshot = await readDb();
    const resolved = resolveUserGroupsAndPermissions(dbSnapshot, session.username);
    req.adminPermissions = resolved?.permissions || buildDefaultPermissionMatrix(req.adminSession.role);
    req.adminGroups = (resolved?.groups || []).map(sanitizeAdminGroupForResponse);
  }

  void logPanelConnection(req, req.adminSession, 'session');
  upsertConnectedPanelClient(req, req.adminSession, 'session', 'http');
...
app.use((req, res, next) => {
  ...
  const permissionKey = inferPermissionKeyFromPath(req.path);
  if (!permissionKey) {
    return next();
  }

  const mode = inferPermissionMode(req.method);
  if (hasAdminPermission(req, permissionKey, mode)) {
    return next();
  }
...
app.use((req, res, next) => {
  ...
  if (String(req.path || '').startsWith('/api/player/')) {
    return next();
  }
  ...
  res.on('finish', () => {
    ...
    const username = req.adminSession?.username || 'anonymous';
    const role = sanitizeAdminRole(req.adminSession?.role);
    ...
    void (async () => {
      try {
        ...
        appendLogEntry(db, {
          type: 'ops',
          level,
          source: 'api.panel.mutation',
          message: `Modification API ${method} ${req.path}`,
          details: buildLogMeta(req, {
            username,
            role,
            statusCode,
            durationMs: Math.max(0, Date.now() - startedAt),
            payloadKeys: summarizePayloadKeys(req.body)
          })
        });
        await writeDb(db);
      } catch (error) {
        appendRuntimeOnlyLog('error', 'api.panel.mutation', 'Impossible de journaliser la modification API.', {
          method,
          path: req.path,
          message: String(error?.message || error)
        });
...
app.post('/api/auth/logout', async (req, res) => {
  ...
  // --- RBAC GROUPS CRUD ---
  app.get('/api/auth/groups', async (req, res) => {
    const db = await readDb();
    const groups = parseAdminGroups(db.kv[ADMIN_GROUPS_KV_KEY]);
    return res.json({ ok: true, groups: groups.map(sanitizeAdminGroupForResponse) });
  });

  app.post('/api/auth/groups', async (req, res) => {
    ...
    if (groups.some((g) => g.name.toLowerCase() === name.toLowerCase())) {
      return res.status(409).json({ ok: false, error: 'name déjà utilisé' });
    }
    const now = nowIso();
    const created = {
      id: crypto.randomUUID(),
      ...
      permissions: sanitizePermissionMatrix(req.body?.permissions, 'viewer'),
      createdAt: now,
      updatedAt: now
    };
    groups.push(created);
    db.kv[ADMIN_GROUPS_KV_KEY] = JSON.stringify(groups);
    await writeDb(db);
    return res.status(201).json({ ok: true, group: sanitizeAdminGroupForResponse(created) });
  });

  app.put('/api/auth/groups/:id', async (req, res) => {
    if (sanitizeAdminRole(req.adminSession?.role) !== 'admin') {
      return res.status(403).json({ ok: false, error: 'admin role required' });
    }
    ...
    groups[idx] = {
      ...groups[idx],
      name: String(req.body?.name || groups[idx].name).trim(),
      description: String(req.body?.description || groups[idx].description).trim(),
      permissions: sanitizePermissionMatrix(req.body?.permissions, 'viewer'),
      updatedAt: nowIso()
    };
    db.kv[ADMIN_GROUPS_KV_KEY] = JSON.stringify(groups);
    await writeDb(db);
    return res.json({ ok: true, group: sanitizeAdminGroupForResponse(groups[idx]) });
  });

  app.delete('/api/auth/groups/:id', async (req, res) => {
    if (sanitizeAdminRole(req.adminSession?.role) !== 'admin') {
      return res.status(403).json({ ok: false, error: 'admin role required' });
    }
    const id = String(req.params?.id || '').trim();
    if (!id) return res.status(400).json({ ok: false, error: 'id requis' });
    const db = await readDb();
    let groups = parseAdminGroups(db.kv[ADMIN_GROUPS_KV_KEY]);
    const idx = groups.findIndex((g) => g.id === id);
    if (idx < 0) return res.status(404).json({ ok: false, error: 'group not found' });
    groups.splice(idx, 1);
    db.kv[ADMIN_GROUPS_KV_KEY] = JSON.stringify(groups);
  ...
  app.get('/api/auth/permissions', (req, res) => {
    return res.json({ ok: true, permissions: PERMISSION_CATALOG });
  });

  // --- CONNECTED CLIENTS ---
  app.get('/api/monitoring/connected-clients', (req, res) => {
    return res.json({ ok: true, clients: listConnectedPanelClients() });
  });

  // --- SYSTEM USAGE ---
  app.get('/api/monitoring/system-usage', async (req, res) => {
    const usage = await sampleSystemUsage();
  ...
  app.get('/api/downloads', (req, res) => {
    ...
    for (const platform of DOWNLOAD_PLATFORMS) {
      let url = resolveDownloadTarget(platform);
      if (!url) {
        // fallback: propose local static file if exists
        const staticPath = path.join(__dirname, 'public', 'downloads', `${platform}.zip`);
        if (fs.existsSync(staticPath)) {
          url = `${baseUrl}/downloads/${platform}.zip`;
        }
      }
      result[platform] = url || null;
    }
    return res.json({ ok: true, downloads: result });
  });
...
app.get('/api/auth/session', async (req, res) => {
  ...
  const db = await readDb();
  const resolved = resolveUserGroupsAndPermissions(db, session.username);
  const role = sanitizeAdminRole(resolved?.user?.role || session.role);

  return res.json({
    ok: true,
    username: session.username,
    role,
    groups: (resolved?.groups || []).map(sanitizeAdminGroupForResponse),
    permissions: resolved?.permissions || buildDefaultPermissionMatrix(role),
    idleTimeoutMs: ADMIN_IDLE_TIMEOUT_MS,
    lastActivityAt: session.lastActivityAt
  });
});

app.get('/api/mobile/connect/auth', async (req, res) => {
  const role = sanitizeAdminRole(req.adminSession?.role);
  return res.json({
    ok: true,
    username: req.adminSession?.username || 'api-key',
    role,
    permissions: req.adminPermissions || buildDefaultPermissionMatrix(role),
    groups: req.adminGroups || [],
    apiKeySession: req.apiKeySession || null
  });
});
...
app.post('/api/auth/users', async (req, res) => {
  ...
  const username = normalizeUsername(req.body?.username);
  const password = String(req.body?.password || '');
  const requestedRole = sanitizeAdminRole(req.body?.role || 'viewer');
  const role = username === 'admin' ? 'admin' : requestedRole;
  ...
  const nextUsers = upsertAdminUserRecord(users, {
    ...created,
    role,
    groups: sanitizeGroupIds(req.body?.groups),
    permissions: sanitizePermissionMatrix(req.body?.permissions, role),
    createdAt: now,
    updatedAt: now
  });
...
app.put('/api/auth/users/:username', async (req, res) => {
  ...
  const nextGroupsRaw = req.body?.groups;
  const nextPermissionsRaw = req.body?.permissions;
  const hasRole = typeof nextRoleRaw === 'string' && nextRoleRaw.trim().length > 0;
  const hasPassword = nextPassword.length > 0;
  const hasGroups = Array.isArray(nextGroupsRaw);
  const hasPermissions = nextPermissionsRaw && typeof nextPermissionsRaw === 'object' && !Array.isArray(nextPermissionsRaw);

  if (!username || (!hasRole && !hasPassword && !hasGroups && !hasPermissions)) {
  ...
  let nextRole = hasRole ? sanitizeAdminRole(nextRoleRaw) : sanitizeAdminRole(current.role);
  if (username === 'admin') {
    nextRole = 'admin';
  }
  ...
  users[idx] = {
    ...
    passwordHash,
    passwordAlgo,
    groups: hasGroups ? sanitizeGroupIds(nextGroupsRaw) : sanitizeGroupIds(current.groups),
    permissions: hasPermissions
      ? sanitizePermissionMatrix(nextPermissionsRaw, nextRole)
      : sanitizePermissionMatrix(current.permissions, nextRole),
    updatedAt: nowIso()
  };
...
app.delete('/api/auth/users/:username', async (req, res) => {
  ...
  if (username === 'admin') {
    return res.status(409).json({ ok: false, error: 'suppression du compte admin principal interdite' });
  }
...
app.get('/api/mobile/connect/requests', async (req, res) => {
  if (sanitizeAdminRole(req.adminSession?.role) !== 'admin') {
    return res.status(403).json({ ok: false, error: 'admin role required' });
  }

  const rows = loadMobileConnectRequests().map((row) => ({
    id: row.id,
    appName: row.appName,
...
app.post('/api/mobile/connect/init', async (req, res) => {
  const appName = String(req.body?.appName || 'Digital Kiosk App').trim().slice(0, 120) || 'Digital Kiosk App';
  const platform = String(req.body?.platform || 'unknown').trim().slice(0, 60) || 'unknown';
  const callbackUrl = String(req.body?.callbackUrl || '').trim().slice(0, 600);
  ...
  const rows = loadMobileConnectRequests();
  rows.push(requestRow);
  saveMobileConnectRequests(rows.slice(0, 300));

  const approvalUrl = `/mobile/connect/approve?requestId=${encodeURIComponent(requestId)}`;

  appendRuntimeOnlyLog('info', 'api.mobile.connect.init', 'Nouvelle demande de connexion mobile.', {
    requestId,
    appName,
    platform,
...
app.get('/api/mobile/connect/status', async (req, res) => {
  const requestId = String(req.query?.requestId || '').trim();
  const pollToken = String(req.query?.pollToken || '').trim();
  if (!requestId || !pollToken) {
    return res.status(400).json({ ok: false, error: 'requestId and pollToken are required' });
  }

  const rows = loadMobileConnectRequests();
  const requestRow = rows.find((row) => row.id === requestId && row.pollToken === pollToken);
  if (!requestRow) {
    return res.status(404).json({ ok: false, error: 'mobile connect request not found' });
  }
  ...
  return res.json({
    ...
    approvalCode: requestRow.status === 'approved' ? requestRow.approvalCode : null,
    rejectReason: requestRow.rejectReason
  });
});

app.post('/api/mobile/connect/exchange', async (req, res) => {
  const requestId = String(req.body?.requestId || '').trim();
  const pollToken = String(req.body?.pollToken || '').trim();
  const approvalCode = String(req.body?.approvalCode || '').trim();
  const state = String(req.body?.state || '').trim();

  if (!requestId || !pollToken || !approvalCode || !state) {
    return res.status(400).json({ ok: false, error: 'requestId, pollToken, approvalCode and state are required' });
  }

  const rows = loadMobileConnectRequests();
  const idx = rows.findIndex((row) => row.id === requestId && row.pollToken === pollToken);
  if (idx < 0) {
    return res.status(404).json({ ok: false, error: 'mobile connect request not found' });
  }

  const requestRow = rows[idx];
  if (requestRow.status !== 'approved') {
    return res.status(409).json({ ok: false, error: 'request not approved' });
  }
  if (requestRow.state !== state || requestRow.approvalCode !== approvalCode) {
    return res.status(401).json({ ok: false, error: 'invalid approval code or state' });
  }

  const { record, plainKey } = createApiKeyRecord({
    name: `${requestRow.appName} (${requestRow.platform})`,
    createdBy: requestRow.approvedBy || 'admin',
    scopes: ['admin-api']
  });

  const apiKeys = loadApiKeys();
  apiKeys.push(record);
  saveApiKeys(apiKeys.slice(0, 1200));
  ...
  rows[idx] = {
    ...
    exchangedAt: nowIso(),
    apiKeyId: record.id,
    apiKeyPreview: `${record.prefix}...`
  };
  saveMobileConnectRequests(rows);

  appendRuntimeOnlyLog('info', 'api.mobile.connect.exchange', 'Clé API créée automatiquement pour un client mobile.', {
    requestId,
    apiKeyId: record.id,
    appName: requestRow.appName,
...
app.get('/mobile/connect/approve', async (req, res) => {
  const requestId = String(req.query?.requestId || '').trim();
  if (!requestId) {
    return res.status(400).type('text/plain').send('requestId manquant.');
  }

  const token = getAdminAuthToken(req);
  const session = token ? touchAdminSession(token) : null;
  if (!session || sanitizeAdminRole(session.role) !== 'admin') {
    const redirectPath = `/mobile/connect/approve?requestId=${encodeURIComponent(requestId)}`;
  ...
  if (!requestRow) {
    return res.status(404).type('text/plain').send('Demande de connexion introuvable.');
  }

  if (requestRow.status !== 'pending') {
    return res.status(200).type('text/html').send(`<!doctype html><html><body style="font-family:Segoe UI;background:#0f172a;color:#e5e7eb;padding:24px;"><h1>Demande déjà traitée</h1><p>Statut actuel: ${requestRow.status}</p></body></html>`);
  }

  return res.type('text/html').send(`<!doctype html><html><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>Approbation mobile</title></head><body style="margin:0;background:#0f172a;color:#e5e7eb;font-family:Segoe UI,Arial,sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;padding:16px;"><div style="max-width:760px;width:100%;border:1px solid rgba(255,255,255,.14);border-radius:18px;padding:20px;background:rgba(255,255,255,.06);"><h1 style="margin:0 0 8px 0;">Approbation de connexion mobile</h1><p style="color:#94a3b8;margin:0 0 16px 0;">Valider cette connexion créera automatiquement une clé API dédiée.</p><ul style="line-height:1.8;"><li><strong>Application</strong>: ${requestRow.appName}</li><li><strong>Plateforme</strong>: ${requestRow.platform}</li><li><strong>Request ID</strong>: ${requestRow.id}</li></ul><form method="post" action="/api/mobile/connect/approve" style="display:grid;gap:12px;"><input type="hidden" name="requestId" value="${requestRow.id}" /><button type="submit" name="decision" value="approve" style="border:1px solid rgba(255,255,255,.16);background:#16a34a;color:white;padding:10px 14px;border-radius:10px;cursor:pointer;">Approuver</button><button type="submit" name="decision" value="reject" style="border:1px solid rgba(255,255,255,.16);background:#dc2626;color:white;padding:10px 14px;border-radius:10px;cursor:pointer;">Refuser</button></form></div></body></html>`);
});

app.post('/api/mobile/connect/approve', express.urlencoded({ extended: false }), async (req, res) => {
  const token = getAdminAuthToken(req);
  const session = token ? touchAdminSession(token) : null;
  if (!session || sanitizeAdminRole(session.role) !== 'admin') {
    const requestIdForRedirect = String(req.body?.requestId || req.query?.requestId || '').trim();
    const wantsHtml = String(req.headers.accept || '').includes('text/html');
    if (wantsHtml) {
      const redirectPath = requestIdForRedirect
        ? `/mobile/connect/approve?requestId=${encodeURIComponent(requestIdForRedirect)}`
        : '/mobile/connect/approve';
      return res.redirect(`/login?redirect=${encodeURIComponent(redirectPath)}`);
    }
  ...
  if (!requestId) {
    return res.status(400).json({ ok: false, error: 'requestId requis' });
  }
  ...
  const requestRow = rows[idx];
  if (requestRow.status !== 'pending') {
    return res.status(409).json({ ok: false, error: 'request already processed', status: requestRow.status });
  }
  ...
  if (decision === 'reject') {
    ...
    saveMobileConnectRequests(rows);
    return res.redirect(`/mobile/connect/approve?requestId=${encodeURIComponent(requestId)}`);
  }

  const approvalCode = crypto.randomBytes(20).toString('base64url');
  rows[idx] = {
    ...requestRow,
    status: 'approved',
    approvalCode,
  ...
  if (requestRow.callbackUrl) {
    try {
      const callback = new URL(requestRow.callbackUrl);
      callback.searchParams.set('requestId', requestRow.id);
      callback.searchParams.set('state', requestRow.state);
      callback.searchParams.set('code', approvalCode);
      return res.redirect(callback.toString());
    } catch {
      // invalid callback URL, keep default response
    }
  }
...
app.post('/api/screens', async (req, res) => {
  ...
  const db = await readDb();
  const screens = getCollectionRecords(db, 'screens');
  const idx = screens.findIndex((item) => item.id === id);
  const now = nowIso();
  ...
  const nextRecord = {
    ...
    ...record,
    id,
    name: String(record.name || (idx >= 0 ? screens[idx].name : id)).trim() || id,
    status: String(record.status || (idx >= 0 ? screens[idx].status : 'pending')).trim() || 'pending',
    roomIds: Array.isArray(record.roomIds)
      ? record.roomIds.map((value) => String(value || '').trim()).filter(Boolean)
      : (idx >= 0 && Array.isArray(screens[idx].roomIds) ? screens[idx].roomIds : []),
    updatedAt: now
  };

  if (idx >= 0) {
    screens[idx] = nextRecord;
  } else {
    screens.push({
      ...nextRecord,
      createdAt: now,
      lastHeartbeat: nextRecord.lastHeartbeat || ''
    });
  }
  ...
  appendLogEntry(db, {
    type: 'screen',
    level: 'info',
    source: 'api.screens.upsert',
    message: `Screen upsert: ${id}`,
    details: buildLogMeta(req, { screenId: id, status: nextRecord.status })
  });
  await writeDb(db);
  return res.json({ ok: true, record: sanitizeScreenForResponse(nextRecord) });
});
...
app.delete('/api/screens/:id', async (req, res) => {
  ...
  const db = await readDb();
  const screens = getCollectionRecords(db, 'screens');
  const next = screens.filter((item) => item.id !== id);
  const removed = screens.length - next.length;
  setCollectionRecords(db, 'screens', next);
  appendLogEntry(db, {
    type: 'screen',
    level: removed > 0 ? 'warning' : 'info',
    source: 'api.screens.delete',
    message: `Screen delete: ${id}`,
    details: buildLogMeta(req, { screenId: id, removed })
  });
  await writeDb(db);
...
app.post('/api/meetings', async (req, res) => {
  ...
  const startAt = String(record.startAt || '').trim();

  if (!id) return res.status(400).json({ ok: false, error: 'record.id is required' });
  if (!title) return res.status(400).json({ ok: false, error: 'record.title is required' });
  if (!startAt || Number.isNaN(new Date(startAt).getTime())) {
    return res.status(400).json({ ok: false, error: 'record.startAt must be a valid ISO date' });
  }

  const db = await readDb();
  const events = getCollectionRecords(db, 'events');
  const index = events.findIndex((item) => String(item?.id || '').trim() === id);
  const nextRecord = {
    ...(index >= 0 ? events[index] : {}),
    ...record,
    id,
    title,
    startAt,
    endAt: String(record.endAt || '').trim() || null,
    roomId: String(record.roomId || '').trim() || null,
    roomNumber: String(record.roomNumber || '').trim() || ''
  };

  if (index >= 0) {
    events[index] = nextRecord;
  } else {
    events.push(nextRecord);
  }

  setCollectionRecords(db, 'events', events);
  appendLogEntry(db, {
    type: 'sync',
    level: 'info',
    source: 'api.meetings.upsert',
    message: `Meeting upsert: ${id}`,
    details: buildLogMeta(req, { meetingId: id, startAt })
  });
  await writeDb(db);
  return res.json({ ok: true, record: nextRecord });
});
...
app.delete('/api/meetings/:id', async (req, res) => {
  ...
  const db = await readDb();
  const events = getCollectionRecords(db, 'events');
  const next = events.filter((item) => String(item?.id || '').trim() !== id);
  const removed = events.length - next.length;
  setCollectionRecords(db, 'events', next);
  appendLogEntry(db, {
    type: 'sync',
    level: removed > 0 ? 'warning' : 'info',
    source: 'api.meetings.delete',
    message: `Meeting delete: ${id}`,
    details: buildLogMeta(req, { meetingId: id, removed })
  });
  await writeDb(db);
  return res.json({ ok: true, removed });
});
...
app.get('/api/system/maintenance/status', async (_req, res) => {
...
app.post('/api/system/maintenance/toggle', async (req, res) => {
  ...
  setMaintenanceMode(enabled, reason || null);
  return res.json({ ok: true, enabled: maintenanceMode, reason: reason || null, updatedAt: nowIso() });
});
...
app.get('/api/system/update/state', async (_req, res) => {
  ...
  if (maintenanceMode && !state.isRunning && (state.completedAt || state.workerStatus === 'completed' || state.workerStatus === 'failed')) {
    setMaintenanceMode(false, 'update-finished');
  }
...
app.post('/api/system/update/execute', async (req, res) => {
  try {
    ...
    setMaintenanceMode(true, 'system-update');
...
app.post('/api/player/pair/start', async (req, res) => {
  ...
  if (existingIndex >= 0) {
    ...
    await writeDb(db);
    return res.json({ ok: true, pin, expiresAt, ip, os: baseEntry.os || 'unknown' });
  }
    ...
    if (pendingCommand && !pendingCommand.ackedAt) {
      ...
      if (retryAllowed && intervalElapsed) {
        ...
        screens[idx] = {
          ...
          networkMbps: Number(telemetry.networkMbps || existing.networkMbps || 0) || 0,
          networkInterface: String(telemetry.networkInterface || existing.networkInterface || '').trim(),
...
async function startServer() {
  ...
  maintenanceMode = Boolean(loadMaintenanceState().enabled);
  await readDb();
  startReleaseWatcher();
  const server = app.listen(PORT, '0.0.0.0', () => {
    serverInstance = server;
    appendRuntimeOnlyLog('info', 'server.lifecycle', 'Serveur démarré.', {
      port: PORT,
      apiOnly: API_ONLY,
      mode: SERVER_MODE,
      dbPath: DB_PATH,
  ...
  server.on('error', (error) => {
    const details = serializeError(error);
    appendRuntimeOnlyLog('error', 'server.listen', details.message || 'Erreur serveur', details);
  });
  server.on('close', () => {
    appendRuntimeOnlyLog('warning', 'server.lifecycle', 'Serveur fermé.', {});
    stopSystemUsageBroadcastLoop();
    serverInstance = null;
  });
