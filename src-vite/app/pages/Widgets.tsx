    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#9ca3af]">Create and configure display widgets</p>
        </div>
        ...
        {widgetTypes.map((widget) => {
          ...
          return (
            <GlassCard key={widget.id} className="p-6 hover:scale-[1.02] transition-all duration-200 cursor-pointer">
              ...
              <p className="text-[#9ca3af] text-sm">0 configured</p>
            </GlassCard>
          );
        })}
      </div>
      ...
      <GlassCard className="p-6">
        <div className="text-center py-20">
          ...
          <h3 className="text-lg text-[#e5e7eb] mb-2">No widgets configured</h3>
          <p className="text-[#9ca3af] mb-6">Create widgets to add dynamic content to your displays</p>
          <GlassButton onClick={() => setShowCreate(true)}>
            <Plus size={20} className="inline mr-2" />
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <GlassCard className="w-full max-w-lg p-6 m-4">
            <h2 className="text-xl text-[#e5e7eb] mb-4">Create Widget</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-[#e5e7eb] mb-2" htmlFor="widget-type">Widget Type</label>
              ...
              <div>
                <label className="block text-[#e5e7eb] mb-2">Widget Name</label>
                <input
                  type="text"
