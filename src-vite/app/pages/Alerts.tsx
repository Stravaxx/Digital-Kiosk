...
export function Alerts() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl text-[#e5e7eb] mb-2">{t('alerts.title')}</h1>
        <p className="text-[#9ca3af]">{t('alerts.subtitle')}</p>
      </div>

      <GlassCard className="p-6 space-y-4">
        <h2 className="text-lg text-[#e5e7eb]">{t('alerts.thresholds')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="text-[#9ca3af] text-sm">{t('alerts.offline')}
          ...
          <label className="text-[#9ca3af] text-sm">{t('alerts.stale')}
          ...
          <label className="text-[#9ca3af] text-sm">{t('alerts.maxTemp')}
          ...
          <label className="text-[#9ca3af] text-sm">{t('alerts.maxStorage')}
        ...
        <GlassButton onClick={saveConfig} disabled={saving}>{saving ? t('alerts.saving') : t('alerts.save')}</GlassButton>
      </GlassCard>

      <GlassCard className="p-6">
        <h2 className="text-lg text-[#e5e7eb] mb-4">{t('alerts.active')} ({alerts.length})</h2>
        <div className="space-y-3">
          {alerts.length === 0 ? (
            <p className="text-[#9ca3af]">{t('alerts.none')}</p>
          ) : alerts.map((item) => (
            <div key={item.id} className={`p-3 rounded-[12px] border ${item.severity === 'critical' ? 'border-[#ef4444]/60 bg-[#ef4444]/10' : 'border-[#f59e0b]/60 bg-[#f59e0b]/10'}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-[#9ca3af] mt-1">{t('alerts.status')}: {item.status || 'new'}</div>
                </div>
